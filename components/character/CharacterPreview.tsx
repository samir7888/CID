...
export default function CharacterPreview({ character, loadModel = true }: { character: CharacterDefinition; loadModel?: boolean }) {
    ...
    const modelKey = `${character.id}:${shouldLoadModel}`;
    const [readyKey, setReadyKey] = useState<string | null>(null);
    const modelReady = readyKey === modelKey;
    const markLoaded = () => setReadyKey(modelKey);
    ...
    return (
        <div className="character-preview-stage">
            <Canvas camera={{ position: [0, 1.05, 3.2], fov: 34 }} dpr={[1, 1.5]}>
                ...
                {character.modelPath ? (
                    <ModelPreview path={character.modelPath} scale={character.modelScale} yOffset={character.modelYOffset} onLoaded={markLoaded} />
                ) : <AgentPreview />}
            </Canvas>
