...
export function getChaserBaseDistance(score: number): number {
  ...
  return (
    CHASER_BASE_DISTANCE -
    aggression * (CHASER_BASE_DISTANCE - CHASER_CLOSE_DISTANCE) * 0.5
  );
}
 ...
 */
export function shouldTriggerCloseIn(
  score: number,
  randomRoll: number
): boolean {
  const aggression = getChaserAggression(score);
...
export function getObstacleSpawnGap(
  score: number,
  minGap: number,
  maxGap: number
): number {
  ...
  const tightened =
    maxGap - (maxGap - minGap) * (band.speedMultiplier - 1) * 1.2;
